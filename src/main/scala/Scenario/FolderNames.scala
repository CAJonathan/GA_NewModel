...
import Scenario.Abstract.FolderOrganizer
import java.io.File
...
object FolderNames extends FolderOrganizer {
  ...
  def determineDataAndImageFilePath(properties: Properties): (String, String) = {
    ...
    energyDistributionName match{
      case FOLDER_NORMAL_DISTRIBUTION_ENERGY =>
        energyDistributionType = Some(properties.getAsString("scenario.generator.energy.distribution.normal.type"))
        locationInFolder += "/" + FolderNames.FOLDER_NORMAL_DISTRIBUTION_ENERGY + "/" + energyDistributionType.get


      case FOLDER_UNIFORM_DISTRIBUTION_ENERGY =>
        locationInFolder += "/" + FolderNames.FOLDER_UNIFORM_DISTRIBUTION_ENERGY

    }

    locationDistributionName match{
      case FOLDER_NORMAL_DISTRIBUTION_LOCATION =>
        locationDistributionType = Some(properties.getAsString("scenario.generator.location.distribution.normal.type"))
        locationInFolder += "/" + FolderNames.FOLDER_NORMAL_DISTRIBUTION_LOCATION + "/" + locationDistributionType.get


      case FOLDER_UNIFORM_DISTRIBUTION_LOCATION =>
        locationInFolder += "/" + FolderNames.FOLDER_UNIFORM_DISTRIBUTION_LOCATION
    ...
    if(locationDistributionType.isDefined){
      locationDistributionType.get match{
        case FOLDER_NORMAL_DISTRIBUTION_LOCATION_FAR =>
          fileName += FILE_NORMAL_DISTRIBUTION_LOCATION_FAR + FILE_DELIMETER_SYNTAX


        case FOLDER_NORMAL_DISTRIBUTION_LOCATION_MEDIUM =>
          fileName += FILE_NORMAL_DISTRIBUTION_LOCATION_MEDIUM + FILE_DELIMETER_SYNTAX


        case FOLDER_NORMAL_DISTRIBUTION_LOCATION_NEAR =>
          fileName += FILE_NORMAL_DISTRIBUTION_LOCATION_NEAR + FILE_DELIMETER_SYNTAX
    ...
    if(energyDistributionType.isDefined){
      energyDistributionType.get match{
        case FOLDER_NORMAL_DISTRIBUTION_ENERGY_HIGH =>
          fileName += FILE_DELIMETER_SYNTAX + FILE_NORMAL_DISTRIBUTION_ENERGY_HIGH


        case FOLDER_NORMAL_DISTRIBUTION_ENERGY_MEDIUM =>
          fileName += FILE_DELIMETER_SYNTAX + FILE_NORMAL_DISTRIBUTION_LOCATION_MEDIUM


        case FOLDER_NORMAL_DISTRIBUTION_ENERGY_LOW =>
          fileName += FILE_DELIMETER_SYNTAX + FILE_NORMAL_DISTRIBUTION_LOCATION_NEAR
